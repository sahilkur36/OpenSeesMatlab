    ...
    methods (Static)
        function opts = defaultOptions()
            ...
            opts.help = strjoin({
                ...
                ''
                '-- General layout ----------------------------------------------'
                '  general.clearAxes   logical  Clear axes before drawing (default true).'
                '  general.holdOn      logical  Hold axes on after drawing (default true).'
                '  general.axisEqual   logical  Equal axis scaling (default true).'
                '  general.grid        logical  Show grid (default true).'
                '  general.box         logical  Show axes box (default false).'
                '  general.view        ''auto''|''xy''|''xz''|''yz''|''iso''  Camera view.'
                '                      auto = 2-D view for planar models, 3-D otherwise.'
                '  general.title       string   Axes title; '''' = no title.'
                '  general.padRatio    double   Axis padding as fraction of model size (default 0.05).'
                '  general.figureSize  [w h]    Figure size in pixels (default [1000 618]).'
                ''
                '-- Style -------------------------------------------------------'
                '  style.mode           ''byFamily'' | ''solid'' | ''wireframe'''
                '                       byFamily  = each element family gets its own colour.'
                '                       solid     = all surfaces use their style.*Color field.'
                '                       wireframe = edges only, no filled surfaces.'
                '  style.lineColor      color   Default line/edge colour (#444444).'
                '  style.shellColor     color   Shell surface colour (byFamily mode).'
                ...
                ''
                '-- Nodes -------------------------------------------------------'
                '  nodes.show              logical  Scatter-plot all nodes (default false).'
                '  nodes.size              double   Marker size in points (default 20).'
                '  nodes.marker            string   Marker shape: ''o'', ''s'', ''^'', etc.'
                '  nodes.filled            logical  Filled markers (default true).'
                '  nodes.edgeColor         color   Marker edge colour.'
                '  nodes.color             color   Marker fill colour.'
                '  nodes.showLabels        logical  Show node tag numbers (default false).'
                '  nodes.labelOffsetScale  double   Label offset as fraction of model size (default 0.01).'
                '  nodes.labelColor        color   Label text colour.'
                '  nodes.fontSize          double   Label font size in points (default 9).'
                ''
                '-- Elements ----------------------------------------------------'
                '  elements.showBeam              logical  Draw beam/frame elements (default true).'
                '  elements.showLink              logical  Draw link/zero-length elements (default true).'
                '  elements.showTruss             logical  Draw truss elements (default true).'
                '  elements.showPlane             logical  Draw plane (2-D) elements (default true).'
                ...
                '  elements.showWireframeOnFaces  logical  Draw mesh edges on surfaces (default true).'
                ...
                '  elements.surfaceAlpha          double   Surface opacity 0-1 (default 0.95).'
                '  elements.wireframeLineWidth    double   Edge line width on surfaces (default 0.8).'
                '  elements.showLabels            logical  Show element tag numbers (default false).'
                '  elements.labelOffsetScale      double   Label offset as fraction of model size (default 0.01).'
                '  elements.labelColor            color   Label text colour.'
                '  elements.fontSize              double   Label font size in points (default 9).'
                ''
                '-- Fixed nodes -------------------------------------------------'
                '  fixed.show      logical  Highlight fixed/constrained nodes (default true).'
                '  fixed.size      double   Marker size in points (default 46).'
                '  fixed.marker    string   Marker shape (default ''s'' = square).'
                '  fixed.filled    logical  Filled marker (default true).'
                '  fixed.edgeColor color   Marker edge colour.'
                '  fixed.color     color   Marker fill colour (default dark red).'
                ''
                '-- MP constraints ----------------------------------------------'
                '  mpConstraint.show       logical  Draw MP constraint lines (default true).'
                '  mpConstraint.lineWidth  double   Line width (default 1.2).'
                '  mpConstraint.lineStyle  string   Line style: ''--'', ''-'', '':'', etc.'
                '  mpConstraint.color      color   Line colour.'
                ''
                '-- Local axes --------------------------------------------------'
                '  localAxes.showBeam    logical  Draw local axes on beam elements (default false).'
                '  localAxes.showLink    logical  Draw local axes on link elements (default false).'
                '  localAxes.axisXColor  color   Local x-axis colour (default red).'
                '  localAxes.axisYColor  color   Local y-axis colour (default green).'
                '  localAxes.axisZColor  color   Local z-axis colour (default blue).'
                '  localAxes.scale       double   Arrow length as fraction of model size (default 0.04).'
                '  localAxes.lineWidth   double   Arrow line width (default 1.2).'
                '  localAxes.labelAxes   logical  Label x/y/z at arrow tips (default false).'
                '  localAxes.fontSize    double   Tip label font size in points (default 8).'
                ''
                '-- Loads -------------------------------------------------------'
                '  loads.showNodal        logical  Draw nodal load arrows (default false).'
                '  loads.showElement      logical  Draw distributed element load arrows (default false).'
                '  loads.showLabels       logical  Show force magnitude text labels (default false).'
                '  loads.scale            double   Manual arrow scale multiplier (default 1.0).'
                '  loads.baseFraction     double   Base arrow length as fraction of model size (default 0.08).'
                '  loads.lineWidth        double   Arrow line width (default 1.4).'
                '  loads.nodalColor       RGB      Nodal arrow colour (default red-ish [0.85 0.20 0.20]).'
                '  loads.elementColor     RGB      Distributed load arrow colour (default green-ish).'
                '  loads.textColor        RGB      Label text colour.'
                '  loads.fontSize         double   Label font size in points (default 10).'
                '  loads.labelOffset      [dx dy dz]  World-space label position offset.'
                '  loads.nElementArrows   int      Number of arrows drawn per element load (default 15).'
                '  loads.normalizeLength  logical  Normalise all arrows to the same length (default false).'
                '  loads.minNorm          double   Minimum force norm to draw an arrow (default 1e-12).'
                ''
                '-- Outline -----------------------------------------------------'
                '  outline.show       logical  Draw model bounding-box outline (default false).'
                '  outline.color      RGB      Outline colour (default grey [0.5 0.5 0.5]).'
                '  outline.lineStyle  string   Outline line style (default '':'').'
                '  outline.lineWidth  double   Outline line width (default 0.9).'
                ''
                '-- Summary -----------------------------------------------------'
                '  summary.show  logical  Print element/node counts to console (default true).'
                ''
                '-- Performance -------------------------------------------------'
                '  performance.maxDenseLookupTag  int  Maximum tag value for direct-array'
                '                                      index lookup; tags above this threshold'
                '                                      fall back to containers.Map (default 5e6).'
                ''
                '== Example ====================================================='
                '  disp(opts.help)                    % print this help'
                '  opts.style.mode         = ''byFamily'';'
                '  opts.elements.showSolid = false;'
